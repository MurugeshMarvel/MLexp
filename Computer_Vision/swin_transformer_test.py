...
def sanity_check(config: DictConfig):
    ...
    gpu_dev = config.trainer.gpus
    
    CUDA_FLAG = True if T.cuda.is_available() and type(gpu_dev) == list else False
    model = build_model(model_config, CUDA_FLAG)
    #logging.debug(f"Built Model {summary(model)}")
    
    model_out = model(tst_img)
    logging.debug(f"Output tensor shape from the model - {model_out.shape}")
...
def train(config: DictConfig):
    ...
    train_dataloader, test_dataloader = get_data(**data_config)
    
    log_text = ''
    min_valid_loss = None 
    for e in range(training_config.num_epoch):
        train_loss = 0.
        logging.info(f"Training on epoch - {e}")
        for img, out in tqdm(train_dataloader):
        ...
        if e % VALIDATE_GAP  == 0:
            ...
            for val_data, val_out in tqdm(test_dataloader):
            ...
            total_valid_loss = valid_loss / len(test_dataloader)
