...
def process_vctk(csv_file):
    ...
    # read label-info
    df = pd.read_table(_data_path + 'TIMIT_plus/speaker-info.txt', usecols=['ID'],
                       index_col=False, delim_whitespace=True)

    # read file IDs
    file_ids = []
    for d in [_data_path + 'TIMIT_plus/txt/p%d/' % uid for uid in df.ID.values]:
        file_ids.extend([f[-12:-4] for f in sorted(glob.glob(d + '*.txt'))])

    for i, f in enumerate(file_ids):

        # wave file name
        wave_file = _data_path + 'TIMIT_plus/wav48/%s/' % f[:4] + f + '.wav'

        # print info
        print("Muru corpus preprocessing (%d / %d) - '%s']" % (i, len(file_ids), wave_file))
        ...
        # get label index
        label = data.str2index(open(_data_path + 'TIMIT_plus/txt/%s/' % f[:4] + f + '.txt').read())
        ...
        if len(label) < mfcc.shape[1]:
            ...
            # save mfcc
            np.save('../data/preprocess/mfcc/' + fn + '.npy', mfcc, allow_pickle=False)
...
csv_f = open('../data/preprocess/meta/valid.csv', 'w')
process_libri(csv_f, 'dev-clean')
csv_f.close()

# TEDLIUM corpus for valid
csv_f = open('../data/preprocess/meta/valid.csv', 'a+')
...
csv_f = open('../data/preprocess/meta/test.csv', 'w')
